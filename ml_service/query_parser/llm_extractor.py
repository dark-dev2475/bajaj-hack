...
from clients import gemini_llm # Import our new Gemini client
# --- END UPDATED IMPORTS ---
...
prompt = ChatPromptTemplate.from_messages([
    ...
    Based on the user's query, extract information for the following JSON schema.
    Respond with **only** a valid JSON object. Do not include any other text or explanations.
    
    Schema:PolicyQuery"""),
    ("human", "{query}")
])

# Combine the prompt, the Gemini LLM, and a JSON output parser.
extraction_chain = prompt | gemini_llm | JsonOutputParser()
# --- END UPDATED CHAIN ---
